...
def print_directory_structure(start_path, indent=0, ignore_dirs=None, ignore_files=None):
    """
    ...
    try:
        ...
        dirs = [item for item in items if os.path.isdir(os.path.join(start_path, item)) and item not in ignore_dirs]
        files = [item for item in items if os.path.isfile(os.path.join(start_path, item)) and item not in ignore_files]
